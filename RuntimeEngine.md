...
* **\[graphics\]** - display mode and various graphics options
  ...
  * `windowed` = \[0; 1\] - when enabled (true), runs game in windowed mode.
  * `screen_def` = \[string\] - determines how the display mode is deduced:
  ...
  * `render_at_screenres` = \[0; 1\] - whether the sprites are transformed and rendered in the game's native or the current display resolution;
...
* **\[sound\]** - sound options
  ...
  * `midiid` = \[string; 0; -1\] - MIDI driver id, '0' or 'none', '-1' or 'auto'. Driver IDs are platform-dependent.
    ...
    * For Windows:
      * `W32M` - MIDI mapper;
      * `W32A` - MIDI driver.
  * `usespeech` = \[0; 1\] - disable or enable in-game speech (voice-overs).
  * `threaded` = \[0; 1\] - when enabled, the engine runs audio on a separate thread.
* **\[mouse\]** - mouse options
  * `auto_lock` = \[0; 1\] - enables the mouse autolock inside the game window: mouse cursor locks inside the window whenever it receives input focus.
  ...
  * `speed_def` = \[string\] - determines how the cursor speed value is interpreted, possible modes are:
    ...
    * `current_display` - keep the cursor's speed relation to the screen size by increasing the actual cursor speed when running the game in low resolution and decreasing the cursor speed when running the game in a resolution higher than the current user's desktop resolution (this is default).
...
* **\[misc\]** - various options
  ...
  * `localuserconf` = \[0; 1\] - read and write user config in the game's directory rather than using the standard system path. The game directory must be writable for this option to work, otherwise the engine will fall back to the standard path.
  * `user_data_dir` = \[string\] - custom path to the savedgames location.
  * `shared_data_dir` = \[string\] - custom path to the shared appdata location.
  * `antialias` = \[0; 1\] - disable / enable anti-alias scaled sprites.
  * `cachemax` = \[integer\] - size of the engine's sprite cache, in kilobytes. Default size is 131072 (128 MB).
* **\[log\]** - log options, allow to setup logging to the chosen OUTPUT with the given log groups and verbosity levels.
  * `[outputname]` = GROUP[:LEVEL][,GROUP[:LEVEL]][,...];
  * `[outputname]` = +GROUPLIST[:LEVEL];<br>
    Groups may be defined either by name or by a LIST of one-letter IDs, preceded by '+', e.g. +ABCD:LEVEL. Verbosity may be defined either by a name or a numeric ID.
    - OUTPUTs are:
      * `stdout`, `file`, `console` (where \"console\" is the internal engine's console);
    - GROUPs are:
...
* **\[override\]** - special options, overriding game behavior.
  * `multitasking` = \[0; 1\] - lock the game in the "single-tasking" or "multitasking" mode. In a nutshell, "multitasking" here means that the game will continue to run when the player switches away from the game window; otherwise it will freeze until the player switches back.
  * `os` = \[string\] - trick the game to think that it runs on a particular operating system. This may come in handy if the game is scripted to play differently depending on the OS. Possible choices are:
    * `dos` - MS DOS;
    * `win` - Windows;
    * `linux` - Linux;
    * `mac` - MacOS.
  * `upscale` = \[0; 1\] - run the game in the "upscale mode". The earlier versions of AGS provided support for "upscaling" low-res games to hi-res. The script API has means for detecting if the game is running upscaled, and game the developer can use this opportunity to setup the game accordingly (e.g. assign hi-res fonts, etc). This option works **only** for games created with versions *before* AGS 3.1.0, with low-res native resolution, such as 320x200 or 320x240, and it may somewhat improve the game's look.
* **\[disabled\]** - special instructions for the setup program hinting to disable particular options or to lock some in a certain state. Ignored by the engine.
  * `render_at_screenres` = \[0; 1\] - locks "Render sprites in screen resolution" in a default state;
  * `speechvox` = \[0; 1\] - locks "Use digital speech pack" in a default state;
  * `filters` = \[0; 1\] - locks "Graphics filter" selection in a default state;
  * \<`filter id`\> - removes particular graphics filter from the selection list;
