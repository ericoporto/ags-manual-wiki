...
Similarily to [while](managedmodifier#while) runs *statements*
...
[switch](managedmodifier#switch-case-statements) immediately. After this script
...
loop restart, if condition is still met, or loop end. If in [for](managedmodifier#for)
...
to add an [import](managedmodifier#import) line to your script header to allow the
...
you need to use a [new](managedmodifier#new) keyword:
...
See Also: [new](managedmodifier#new), [Pointers in AGS](Pointers)
...
See Also: [managed](managedmodifier#managed), [Pointers in AGS](Pointers)
...
When you are creating custom [structs](managedmodifier#struct), you use the "this" keyword
...
global script with the [export keyword](managedmodifier#export).
...
Normally, if a [while](managedmodifier#dowhile) loop runs for more than
